...
const Table = ({ title, data, onDelete, pagination, page, setPage, limit, search, handleSearchChange, handleLimitChange, handleNextPage, handlePreviousPage, handleExport }) => {
    const header = ['No', 'Parent Department', 'Department Code', 'Department Name', 'Action'];;
...
<TableCell>{row.name}</TableCell>
            ...
            <TableCell className="flex gap-2">
                <Button
                    variant="outline"
                    onClick={() => window.location.href = `/pulse/department/edit/${row.id}`}
                >
                    Edit
                </Button>
                <Button
    ...
    return (
        <>
            <div className="container mx-auto py-4 px-5">
                <DataTable
                    data={data}
                    linkCreate='/pulse/department/create'
                    header={header}
                    body={renderBody}
                    onDelete={onDelete}
