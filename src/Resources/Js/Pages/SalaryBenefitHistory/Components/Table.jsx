...
const Table = ({ title, data, onDelete, pagination, page, setPage, limit, search, handleSearchChange, handleLimitChange, handleNextPage, handlePreviousPage, handleExport }) => {
    const header = ['No', 'Employee', 'Component', 'Contract', 'New Benefit Value', 'Old benefit Value', 'Action'];;
...
<TableCell>{row.new_benefit_value}</TableCell>
<TableCell>{row.old_benefit_value}</TableCell>

            <TableCell className="flex gap-2">
                <Button
                    variant="outline"
                    onClick={() => window.location.href = `/pulse/salary-benefit-history/edit/${row.id}`}
                >
                    Edit
                </Button>
                <Button
    ...
    return (
        <>
            <div className="container mx-auto py-4 px-5">
                <DataTable
                    data={data}
                    linkCreate='/pulse/salary-benefit-history/create'
                    header={header}
                    body={renderBody}
                    onDelete={onDelete}
