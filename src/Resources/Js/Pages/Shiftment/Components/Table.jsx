...
const Table = ({ title, data, onDelete, pagination, page, setPage, limit, search, handleSearchChange, handleLimitChange, handleNextPage, handlePreviousPage, handleExport }) => {
    const header = ['No', 'Code Shiftment', 'Shiftment Name', 'Start Hour', 'End Hour', 'Action'];;
        ...
        return (
            <>
                ...
                <TableCell className="flex gap-2">
                    <Button
                        variant="outline"
                        onClick={() => window.location.href = `/pulse/shiftment/edit/${row.id}`}
                    >
                        Edit
                    </Button>
                    <Button
    ...
    return (
        <>
            <div className="container mx-auto py-4 px-5">
                <DataTable
                    data={data}
                    linkCreate='/pulse/shiftment/create'
                    header={header}
                    body={renderBody}
                    onDelete={onDelete}
