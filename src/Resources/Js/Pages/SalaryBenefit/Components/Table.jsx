...
const Table = ({ title, data, onDelete, pagination, page, setPage, limit, search, handleSearchChange, handleLimitChange, handleNextPage, handlePreviousPage, handleExport }) => {
    const header = ['No', 'Employee', 'Component Salary', 'Benefit value', 'Action'];;
...
<TableCell>{row.benefit_value}</TableCell>
            ...
            <TableCell className="flex gap-2">
                <Button
                    variant="outline"
                    onClick={() => window.location.href = `/pulse/salary-benefit/edit/${row.id}`}
                >
                    Edit
                </Button>
                <Button
    ...
    return (
        <>
            <div className="container mx-auto py-4 px-5">
                <DataTable
                    data={data}
                    linkCreate='/pulse/salary-benefit/create'
                    header={header}
                    body={renderBody}
                    onDelete={onDelete}
