...
const Table = ({ title, data, onDelete, pagination, page, setPage, limit, search, handleSearchChange, handleLimitChange, handleNextPage, handlePreviousPage, handleExport }) => {
    const header = ['No', 'Parent Job Level', 'Job Level Code', 'Job Level Name', 'Action'];;
...
<TableCell>{row.name}</TableCell>
            ...
            <TableCell className="flex gap-2">
                <Button
                    variant="outline"
                    onClick={() => window.location.href = `/pulse/job-level/edit/${row.id}`}
                >
                    Edit
                </Button>
                <Button
    ...
    return (
        <>
            <div className="container mx-auto py-4 px-5">
                <DataTable
                    data={data}
                    linkCreate='/pulse/job-level/create'
                    header={header}
                    body={renderBody}
                    onDelete={onDelete}
